...
| **M3 見どころの抽出** | **設計のみ。** 実装はこれから（`m3-plan.md` §2の5ルール） |
...
1. **飛んで確かめる**（遠景の見た目・キーコンフィグ・3km四方の広さ）。判断待ちが一番の律速
2. **M3の実装**（`m3-plan.md` §2の5ルール）。設計は済んでいて、属性表も揃っている
3. fpsの測り直し（§3.1）。機械が空いた時にまとめて
4. コース生成（`m3-plan.md` §5）。M3のスポットが出てから
