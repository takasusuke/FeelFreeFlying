...
|---|---|---|---|---|---|---|---|---|---|
| 主要地物単位 | 8,226 | 705.4 | 189.4 | 17.6 | **100.0%** | 未計測 | 595,909 | 未計測 | 500.4 MB |
| 地域単位 | | | | | | | | | |

主要地物単位の補足: 0.1% low 48.8 fps。`min 17.6`は計測開始直後の1フレームで、
60fps未満は59,037フレーム中1フレームのみ。ドローコールとメモリは
Profilerが要る（ビルドでは`UnityStats`が使えない → §4.2）ため、地域単位と揃えてEditorで別途取る。
